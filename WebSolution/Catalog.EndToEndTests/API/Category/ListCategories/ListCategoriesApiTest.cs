...
using Catalog.EndToEndTests.Models;
    ...
    : IDisposable
{
    ...
    public async Task ListCategoriesAndTotalByDefault()
    {
        ...
        foreach (CategoryModelOutput outputItem in output.Data)
        {
            ...
            outputItem.CreatedAt.TrimMillisseconds().Should().Be(
                exampleItem.createTime.TrimMillisseconds()
            );
        }
    }
    ...
    [Xunit.Theory(DisplayName = nameof(ListPaginated))]
    [Trait("EndToEnd/API", "Category/List - Endpoints")]
    [InlineData(10, 1, 5, 5)]
    [InlineData(10, 2, 5, 5)]
    [InlineData(7, 2, 5, 2)]
    ...
    [Xunit.Theory(DisplayName = nameof(SearchByText))]
    [Trait("EndToEnd/API", "Category/List - Endpoints")]
    ...
    [Xunit.Theory(DisplayName = nameof(ListOrdered))]
    [Trait("EndToEnd/API", "Category/List - Endpoints")]
    [InlineData("name", "asc")]
    ...
    )
    {
        ...
        var count = 0;
        var expectedArr = expectedOrderedList.Select(x => $"{++count} {x.Name} {x.createTime} {JsonConvert.SerializeObject(x)}");
        count = 0;
    ...
    [Xunit.Theory(DisplayName = nameof(ListOrderedDates))]
    [Trait("EndToEnd/API", "Category/List - Endpoints")]
